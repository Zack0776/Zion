(function () {
  ...
  var PROJECTS = [
    {
      ...
      image: "assets/work-deda-doces.jpg",
      description: "Digital experience for DEDA DOCES — a premium dessert brand website focused on visual storytelling, products and ordering experience.",
      url: "https://deda-doce.vercel.app/"
    },
    {
      title: "DANI PINHEIRO — DAP",
      category: "Personal Branding",
      image: "assets/work-dani-pinheiro-dap.jpg",
      description: "A cinematic digital experience for Dani Pinheiro, built around the DAP method, personal branding and communication, behavior and image.",
      url: "https://danipinheiro.vercel.app/"
    },
  ];
  ...
  var PLACEHOLDERS = { mode: 'never', min: 4 };
  ...
  function projectCell(p, i, featured) {
    ...
    if (featured) { var num = document.createElement('span'); num.className = 'work__num'; num.setAttribute('aria-hidden', 'true'); num.textContent = ('0' + (i + 1)).slice(-2); meta.appendChild(num); }
  ...
  function renderWork() {
    ...
    /* poucos cases reais (sem placeholders) → grade generosa de 2 colunas,
       para cada projeto ganhar presença visual grande e cinematográfica */
    var featured = list.length > 0 && list.length <= 2 && empties === 0;
    var frag = document.createDocumentFragment();
    var n = 0;
    list.forEach(function (p) { frag.appendChild(projectCell(p, n++, featured)); });
    for (var k = 0; k < empties; k++) frag.appendChild(emptyCell(n++));
    while (grid.firstChild) grid.removeChild(grid.firstChild);
    grid.appendChild(frag);
    grid.classList.toggle('work-grid--feature', featured);
  }
