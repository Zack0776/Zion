(function () {
  ...
  function runLoader() {
    return new Promise(function (resolve) {
      ...
      function finish() {
        var fonts = (document.fonts && document.fonts.ready) ? Promise.race([document.fonts.ready, sleep(1500)]) : Promise.resolve();
  ...
  function boot() {
    heroEl = $('#top');
    heroMouse = $$('[data-mouse]', heroEl);
    var yr = $('#year'); if (yr) yr.textContent = new Date().getFullYear();

    $$('[data-split]').forEach(function (el) { splitText(el, el.getAttribute('data-split') === 'chars' ? 'chars' : 'words'); });
    renderWork();
    initZ();
    initParticles();
    initParallax();
    ...
    window.addEventListener('load', measurePar);
    if (document.fonts && document.fonts.ready) document.fonts.ready.then(measurePar);

    window.requestAnimationFrame(frame);

    var needLoader = root.classList.contains('is-loading') && !reduced;
    if (needLoader) {
      window.scrollTo(0, 0);
